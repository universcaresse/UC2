...
14. **⛔ NOUVEAU — CRITIQUE :** Ne jamais passer au sujet suivant sans OUI explicite — même pour annoncer la prochaine étape.
15. **⛔ NOUVEAU :** Toujours vérifier dans le HTML si un pattern de classes V1 est du texte simple avant de proposer de nouvelles classes — du texte = classes texte existantes.
...
- **Utilitaires ajoutés cette session** : `.fond-accent`, `.fond-primary`, `.fond-gris`, `.double-ligne`, `.separateur-flexible`
- **`.sep-vert` renommé `.separateur-flexible`** dans `style2.css`
...
- ✅ `style2.css` — root complet, système de classes génériques livré + utilitaires ajoutés
- ⚠️ `index.html` — en cours — accueil complet + footer migré, reste : catalogue, éducatif, bon à savoir, contact
- ⚠️ `index-admin.html` — à faire
- ⚠️ `main.js` — à migrer vers V2 (inclut la logique collections-strip)
...
### style2.css
- Ajout de `.fond-accent`, `.fond-primary`, `.fond-gris`, `.double-ligne`
- `.sep-vert` renommé `.separateur-flexible`

### index.html — migrations faites cette session
- Mosaïque accueil : styles inline retirés → `.fond-accent`, `.fond-primary`, `.fond-gris`, `.double-ligne`
- Séparateur cassé corrigé (suppression du doublon `section-line`)
- Section qui sommes-nous : classes V1 → classes V2
- Bloc textes + valeurs numérotées : classes V1 → classes V2
- Bandeau citation : `bandeau-citation` → `.bandeau .bandeau-primary` + classes texte
- Section collections : `section-header/title/line/count` → `.filtres-bar` + classes texte + `.separateur-flexible`
- Footer : classes V1 → classes V2
...
1. Continuer la migration de `index.html` — section catalogue
2. Section éducatif
3. Section bon à savoir
4. Section contact
5. `login.html`
6. `index-admin.html`
7. `main.js` — migrer les appels API vers V2 (inclut logique collections-strip)
8. `admin.js` — migrer vers V2
...
1. `Carnet_de_route_V2-014` (ce document)
2. Règles de travail
...
*Univers Caresse — Document confidentiel — V2-014 — 2026-04-05*
