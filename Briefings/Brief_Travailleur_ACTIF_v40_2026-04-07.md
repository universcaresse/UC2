...
## VIOLATIONS COMMISES LORS DE LA SESSION DU 7 AVRIL 2026 — SUITE 2
*(À transmettre à Anthropic via le bouton thumbs down)*

10. Livrer deux changements dans le même message sans attendre le OK entre les deux (HTML bouton + retrait bouton existant)
11. Mettre du style inline dans le HTML (`style="opacity:0"`) — interdit absolu
12. Ne pas lire les fichiers correctement — problèmes évidents (bouton mal placé, tuiles visibles au départ, span hero-seq dans bouton) non détectés avant plusieurs allers-retours
13. Proposer des corrections partielles au lieu de voir le problème global
14. Coder sans attendre le OUI explicite à plusieurs reprises dans la même session
...
## 📍 OÙ ON EST RENDU — fin session 7 avril 2026 (v40)

### Séquence hero accueil — EN COURS — non terminée
Objectif : animation séquentielle en 8 étapes
1. Page vide (fond blanc, nav)
2. Logo + labels stats glissent ensemble
3. Bouton vert se place
4. Tuiles mosaïque apparaissent en fondu une après l'autre
5. "Découvrir les collections" glisse dans le bouton
6. Surtitre "Collections 2026" glisse (du Sheet)
7. 3 stats glissent une après l'autre (du Sheet)
8. Textes sur les tuiles glissent (du Sheet)

**État actuel des fichiers :**
- `index.html` — hero modifié, bouton mal placé (après les stats au lieu d'avant), à corriger
- `main.js` — séquence en place mais délais trop courts, `hero-bouton-texte` est un span.hero-seq dans le bouton = problème
- `style.css` — `.hero-seq`, `.hero-seq-fondu`, `.hero-seq-stat` ajoutés

**Problèmes connus à régler :**
- Bouton doit être AVANT les stats dans le HTML
- `span#hero-bouton-texte` ne doit PAS avoir la classe `hero-seq` — ça interfère avec le layout du bouton
- Les tuiles `.hero-seq-fondu` semblent visibles au départ — à vérifier
- Délais trop courts — tout arrive presque en même temps

*Univers Caresse — Confidentiel — v40 — 7 avril 2026*
